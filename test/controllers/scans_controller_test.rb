...
class ScansControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create scan" do
    assert_difference('Scan.count') do
      post scans_url, params: { scan: { sitemap: @scan.sitemap, status: @scan.status, url: @scan.url } }
    end
  ...
  test "should update scan" do
    patch scan_url(@scan), params: { scan: { sitemap: @scan.sitemap, status: @scan.status, url: @scan.url } }
